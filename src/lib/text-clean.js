...
export function cleanPostText(raw) {
  ...
  return lines
    ...
    .replace(LONG_DASH, '-')
    .trim()
    // Хвостовой ряд дефисов в готовый пост не идёт. В промте он есть и нужен: по паре
    // рядов код находит границы рекламного блока, чтобы дописать его, если модель
    // забыла. Но в промте клиента такой строки не было, и в опубликованном тексте
    // она выглядит мусором — модель копирует её из промта дословно.
    .replace(/\n\s*-{4,}\s*$/, '');
}
