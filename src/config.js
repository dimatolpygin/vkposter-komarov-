...
export const config = {
  ...
  // Адрес журнала сбоев. Раздел «Ошибки» показывает тела ответов провайдеров — там
  // видны и причины отказов, и куски внутренностей системы. Клиенту в панели это не
  // нужно, а вход в панель один. Поэтому путь задаётся переменной: на проде это
  // непубличная строка, ссылки на раздел из меню и с обзора убираются, а знающий адрес
  // заходит напрямую. Пустое значение оставляет привычный `/errors` — так удобно локально.
  diagPath: env('DIAG_PATH', { fallback: 'errors' })
    .replace(/^\/+|\/+$/g, '')
    || 'errors',
