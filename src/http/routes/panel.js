...
export function panelRouter() {
    ...
    try {
      ...
      res.type('html').send(
        page({
          ...
          user: req.user,
          heading: 'Настройки',
          sub: 'Наверху то, что меняют чаще всего. Остальное — в «Тонкой настройке» ниже.',
          message: buildSourceMessage(req.query),
          body,
        }),
      );
  ...
  // секунды, человек в панели ждёт результат. Плановые публикации идут не отсюда, а
  // прогоном по расписанию (`services/run-cycle.js`) — здесь ручная отправка одного поста.
